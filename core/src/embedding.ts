...
import { getEmbeddingModelSettings, getEndpoint } from "./models";
...
export const getEmbeddingConfig = (): EmbeddingConfig => ({
    dimensions:
        settings.USE_OPENAI_EMBEDDING?.toLowerCase() === "true"
            ? (getEmbeddingModelSettings(ModelProviderName.OPENAI) as any)
                  .dimensions
            : settings.USE_OLLAMA_EMBEDDING?.toLowerCase() === "true"
              ? (getEmbeddingModelSettings(ModelProviderName.OLLAMA) as any)
                    .dimensions
              : settings.USE_GAIANET_EMBEDDING?.toLowerCase() === "true"
                ? (getEmbeddingModelSettings(ModelProviderName.GAIANET) as any)
                      .dimensions
                : settings.USE_HEURIST_EMBEDDING?.toLowerCase() === "true"
                  ? (getEmbeddingModelSettings(ModelProviderName.HEURIST) as any)
                        .dimensions
                  : 384, // BGE
    model:
        settings.USE_OPENAI_EMBEDDING?.toLowerCase() === "true"
