...
class GetIPDetailsAction {
    async getIPDetails(
        ...
        Logger.log("Fetching from", `${API_URL}/${RESOURCE_TYPE.ASSET}`);
...
export const getIPDetailsAction = {
    ...
    handler: async (
        ...
        Logger.log("Starting GET_IP_DETAILS handler...");
        ...
        const content = await generateObjectDeprecated({
            runtime,
            context: composeContext({ state: currentState as State, template: getIPDetailsTemplate }),
        ...
        try {
            ...
            Logger.error("Error fetching IP details:", e.message);
            callback?.({
