...
import { settings, Logger } from "@hiveai/utils";
...
export const executeSwap = async (
    ...
    try {
        ...
        if (confirmation.value.err) {
            Logger.log("Confirmation error", confirmation.value.err);
...
export const Sell = async (baseMint: PublicKey, wallet: Keypair) => {
    try {
        ...
        if (!sellTransaction) {
            Logger.log("Failed to get sell transaction");
            return null;
        }
        ...
        if (simulateResult.value.err) {
            Logger.log("Sell Simulation failed", simulateResult.value.err);
            return null;
        }
...
export const Buy = async (baseMint: PublicKey, wallet: Keypair) => {
    try {
        ...
        // simulate the transaction
        if (!buyTransaction) {
            Logger.log("Failed to get buy transaction");
            return null;
        }
        ...
        if (simulateResult.value.err) {
            Logger.log("Buy Simulation failed", simulateResult.value.err);
            return null;
        }
...
export const fetchBuyTransaction = async (
    ...
    try {
        ...
        Logger.log("Failed to get buy transaction", error);
        return null;
    }
};
...
export const fetchSellTransaction = async (
    ...
    try {
        ...
        Logger.log("Failed to get sell transaction", error);
        return null;
    }
};
