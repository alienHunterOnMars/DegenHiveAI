...
import { TrustScoreDatabase } from "@hiveai/plugin-trustdb";
...
async function handler(runtime: any, message: Memory) {
    Logger.log("Evaluating for trust");
    ...
    if (runtime.getSetting("POSTGRES_URL")) {
        Logger.warn("skipping trust evaluator because db is postgres");
        return [];
    }
    ...
    if (!shouldProcess) {
        Logger.log("Skipping process");
        return [];
    }

    Logger.log("Processing recommendations");
    ...
    for (const rec of filteredRecommendations) {
        ...
        if (!rec.contractAddress) {
            ...
            if (!tokenAddress) {
                ...
                rec.contractAddress = tokenAddress;
                if (!tokenAddress) {
                    Logger.warn("Could not find contract address for token");
                    continue;
                }
            }
        }
        ...
        const user = participants.find(async (actor: any) => {
        ...
        if (!user) {
            Logger.warn("Could not find user: ", rec.recommender);
            continue;
        }
        ...
        Logger.log("recommendationsManager", rec);

        // - from here we just need to make sure code is right

        // buy, dont buy, sell, dont sell

        const buyAmounts: any = await tokenProvider.calculateBuyAmounts();
