    ...
    async getRecommendations(
        ...
        const result = Object.keys(groupedRecommendations).map(
            (tokenAddress) => {
                ...
                tokenRecommendations.forEach((recommendation: any) => {
...
export const trustScoreProvider: Provider = {
    async get(
        ...
        try {
            ...
            if (!userId) {
                Logger.error("User ID is missing from the message");
                return "";
            }
            ...
            Logger.error("Error in trust score provider:", error.message);
